# ADR-0002: Agent registries on Soroban, compatible with Stellar 8004
...
2. **Both are interface-compatible with Stellar 8004** (inspected at commit [`d92c2f4`](https://github.com/trionlabs/stellar-8004/tree/d92c2f4ee01858b6da9bf4404ac49322c324958b)). For every function in scope we keep the same name, argument names and order, argument types, event names and fields, and error codes. A client written for Stellar 8004 works against our contract IDs.
3. **Compatibility has three documented exceptions:**
   ...
   - **One puls3 addition:** an agent URI can be registered only once (`UriAlreadyRegistered`), plus a lookup `agent_id_by_uri`. This is the "duplicate registration" error #13 asks for. ERC-8004 lets one owner hold many agents, so a duplicate is defined by the URI, not by the owner.
...
Identical to the Stellar 8004 Reputation Registry for every function below, including errors and events. puls3's "only paid hires count" rule is applied by choosing **which clients to aggregate** (`client_addresses`), not by changing the contract. See open question 4 in the spike.
...
impl ReputationRegistryContract {
    ...
    /// Identity Registry). Stores feedback under the next index for (agent, caller),
...
| `Feedback(u32, Address, u64)` | Persistent | `FeedbackData` | Per feedback; must not be lost |
...
**TTL rules.** Every write extends the instance and each persistent key it writes. `extend_ttl()` extends the instance.

### Implementation notes

Checked on 2026-09-23 by compiling both interfaces above, with stub bodies, against `soroban-sdk` 28.0.0 (`cargo check` and `cargo clippy --all-targets -- -D warnings`):

- The signatures, errors, and events compile as written.
- `give_feedback` and `append_response` have more than 7 arguments, which clippy rejects under `-D warnings`, also in the code generated by `#[contractimpl]`. An `#[allow]` on the `impl` block is **not** enough. #14 adds `#![allow(clippy::too_many_arguments)]` at the top of the crate's `lib.rs`. The argument lists stay as they are, because they are part of the compatible interface.
...
- Code written against Stellar 8004 (its TypeScript SDK, or our own Dart adapter in #17 and #18) works against our contracts by changing the contract ID. The reverse also holds: if we later move to the Stellar 8004 deployments, the server and app change configuration, not code.
...
1. **Use the Stellar 8004 contracts already deployed.** Fastest, and puls3 agents would appear in stellar8004.com and its tools right away. Rejected for now: the team would have no Soroban contract of its own for the hackathon evaluations, and we would depend on a third party's admin and upgrade keys. Kept open as a later migration, which the compatible interface makes cheap.
