...
- **Decision:** puls3 deploys **its own** Identity and Reputation registries that are **interface-compatible with Stellar 8004**: same function names, argument order, types, events, and error codes for everything the MVP uses. Validation stays out of the MVP. Details and the exact interface are in [ADR-0002](../adr/0002-agent-registry-on-soroban.md).
...
|---|---|---|
| [**trionlabs/stellar-8004**](https://github.com/trionlabs/stellar-8004) | All three ERC-8004 registries on Soroban, deployed on [testnet](https://stellar.expert/explorer/testnet/contract/CDE3K4COIAGWNNJQQLL26SYI3KBJF5FUDHXG5FA6GYDJCG7T5V7FIWZH) and mainnet. Identity is an NFT built on OpenZeppelin `stellar-tokens`; timelocked upgrades; reproducible builds; explorer at stellar8004.com; TypeScript SDK; Supabase indexer. MIT. Inspected at commit `d92c2f4`. | **Adopt its interface.** It is the de-facto ERC-8004 ABI on Stellar, so matching it makes our registries usable by the same clients and lets us move to its deployed contracts later by changing a contract ID. We do not fork its code: see "Why not reuse the code" below. |
...
| **Events** | Solidity events with indexed fields | `#[contractevent]` structs; `#[topic]` fields are the indexed ones. Same event names and topic order as Stellar 8004 (`Registered`, `UriUpdated`, `MetadataSet`, `NewFeedback`, ...). | [Events example](https://developers.stellar.org/docs/build/smart-contracts/example-contracts/events) |
| **Storage** | Contract storage, paid once | Three storage types. **Instance** for contract-wide config (admin, counters, linked registry). **Persistent** for per-agent and per-feedback data, which must never be lost. No **temporary** storage. | [Choosing the right storage](https://developers.stellar.org/docs/build/guides/storage/choosing-the-right-storage) |
| **Rent / TTL** | None: data lives forever | Every entry has a TTL and is archived when it expires. Writes and reads extend the TTL of the touched keys (threshold 518,400 ledgers ≈ 30 days, bump to 1,036,800 ≈ 60 days, the same values as Stellar 8004), and a public `extend_ttl(agent_id)` lets anyone keep an agent alive. | [State archival](https://developers.stellar.org/docs/learn/fundamentals/contract-development/storage/state-archival) |
| **Upgrades** | Proxy pattern | Soroban can replace a contract's WASM in place (`update_current_contract_wasm`). **Not in the MVP:** on testnet we redeploy and update `contracts/deployments/testnet.json` (#15). Before mainnet, add an admin-gated, timelocked upgrade like Stellar 8004's. | [Upgrading contracts](https://developers.stellar.org/docs/build/guides/conventions/upgrading-contracts) |
...
1. **Discoverability in the Stellar 8004 ecosystem.** Tools such as stellar-agent-search read the stellar8004.com indexer, which only indexes the Stellar 8004 contracts. Do we ask the Stellar 8004 team to index our registry, register puls3 agents in their registry as well, or move to their contracts after the hackathons? Our compatible interface keeps all three options open.
...
4. **Reputation filtering for puls3.** `get_summary` only aggregates the clients passed in, capped at 5 by Stellar 8004. puls3's "only paid hires count" rule can be enforced by passing the addresses of verified payers, or by computing the summary off-chain from `NewFeedback` events. #11 and #14 decide.
