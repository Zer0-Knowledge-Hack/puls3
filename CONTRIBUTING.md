...
| Area label | Folder | Status |
|---|---|---|
| `area: docs` | `docs/` | Exists |
| `area: architecture` | `docs/adr/`, `docs/architecture/`, `docs/spikes/` | Created by #5, #6, #7 |
| `area: domain` | `puls3_domain/` (pure Dart package) | Placeholder, finalized by #5 |
| `area: contracts` | `contracts/` (Cargo workspace) | Placeholder, created by #12 |
| `area: backend` | `puls3_server/` | Exists |
| `area: blueprints` | `docs/blueprints/` | Placeholder |
| `area: frontend` | `puls3_flutter/` | Exists |
| `area: infra` | `.github/`, `scripts/` | Exists |

`puls3_client/` is generated by Serverpod and has no area of its own: changes to it come from `serverpod generate` in a `backend` PR. ADR-0001 (#5) finalizes this table.
